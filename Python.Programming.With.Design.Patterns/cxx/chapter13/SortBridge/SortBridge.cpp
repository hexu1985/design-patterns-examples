...
// Sorted listbox visual list - 对应Python的SortVisList
class SortVisList : public QListWidget, public VisList {
    Q_OBJECT
public:
    SortVisList(QWidget* frame) : QListWidget(frame) {}
    
    // 排序并按字母顺序添加
    void addLines(const QList<Product*>& prodlist) override {
        QList<Product*> sortedProds = sortUpwards(prodlist);
        for (Product* prod : sortedProds) {
            addItem(prod->getName());
        }
    }
    
    // 按字母顺序排序数据
    QList<Product*> sortUpwards(const QList<Product*>& prodlist) {
        prodList = prodlist;
        QList<Product*> prods;
        ...
        for (int i = 0; i < prods.size(); i++) {
            for (int j = i; j < prods.size(); j++) {
                if (prods[i]->name > prods[j]->name) {
                    Product* ptemp = prods[i];
                    prods[i] = prods[j];
                    prods[j] = ptemp;
                }
            }
        }
        return prods; // 返回排序后的列表
    }
...
public:
    ...
    void build() {
        ...
        // 创建排序列表
        SortVisList* vislist = new SortVisList(lframe);
