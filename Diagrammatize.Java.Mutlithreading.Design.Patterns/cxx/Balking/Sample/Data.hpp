...
public:
    ...
    void change(const std::string& newContent) {
        ...
        std::cout << current_thread::get_thread_name() << " calls change" << std::endl;
        content = newContent;
        changed = true;
    }
    ...
    void save() {
        std::lock_guard<std::mutex> lock(mtx);
        std::cout << current_thread::get_thread_name() << " calls save" << std::endl;
        if (!changed) {
            std::cout << current_thread::get_thread_name() << " needn't calls doSave" << std::endl;
            return;
        }
        doSave();
        changed = false;
